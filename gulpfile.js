...
gulp.task( 'watch', function() {
    ...
    gulp.watch( [`${paths.dev}/js/**/*.js`, 'js/**/*.js', '!js/gorsetalk.js', '!js/gorsetalk.min.js'], gulp.series('scripts') );
...
gulp.task( 'cssnano', function() {
  return gulp.src( `${paths.css}/gorsetalk.css` )
    .pipe( sourcemaps.init( { loadMaps: true } ) )
    .pipe( plumber( {
            errorHandler: function( err ) {
                console.log( err );
...
gulp.task( 'cleancss', function() {
  return gulp.src( `${paths.css}/*.min.css`, { read: false } ) // Much faster
    .pipe( ignore( 'gorsetalk.css' ) )
    .pipe( rimraf() );
});

gulp.task( 'styles', gulp.series( 'sass','cssnano','minifycss' ));
...
gulp.task( 'scripts', function() {
  ...
  gulp.src( scripts, { allowEmpty: true } )
    .pipe( concat( 'gorsetalk.min.js' ) )
    .pipe( uglify() )
    .pipe( gulp.dest( paths.js ) );

  return gulp.src( scripts, { allowEmpty: true } )
    .pipe( concat( 'gorsetalk.js' ) )
    .pipe( gulp.dest( paths.js ) );
});
...
gulp.task( 'watch-bs', gulp.parallel('browser-sync', 'watch', 'scripts','styles'));
